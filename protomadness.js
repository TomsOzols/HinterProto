...
function getSubjectMapData(data) {
    if (data.predicate == "http://www.w3.org/ns/r2rml#class")
        return {
            ontClass: data.object
        }
    else
        return {
            templete: data.object
        }
}
    ...
    if (data.predicate == "http://www.w3.org/ns/r2rml#predicate")
        ...
        _.each(tmpData[data.object], function(obj) {
            ...
            if (obj.predicate == "http://www.w3.org/ns/r2rml#parentTriplesMap") {
                ...
                _.extend(rez, {
                    parentTriplesMap: {
                        name: parentTriplesMap.name,
                        logicalTable: parentTriplesMap.logicalTable,
                        subjectMap: parentTriplesMap.subjectMap
                    }
                });
            }
...
VQ_Schema = function() {
    ...
    if (!this.namespace) {
        defaultOntology = _.max(this.Ontologies, function(ont) {
            return ont.count
        });
...
VQ_Schema.prototype = {
    ...
    ontologyExist: function(name) {
        var ontology = _.find(this.Ontologies, function(ont) {
            if (ont.namespace == name) {
                ont.count = ont.count + 1;
                return ont
            };
        });
        return ontology;
    ...
    getAllClasses: function() {
        return _.map(this.Classes, function(cl) {
            if (cl.isUnique) return {
                name: cl.localName
            };
            else if (cl.ontology.prefix == "") return {
                name: cl.localName
            };
            else return {
                name: cl.ontology.prefix + ":" + cl.localName
            };
        });
    },
    ...
    findSchemaRoleByName: function(name, sourceClass, targetClass) {
        var element = _.find(this.SchemaRoles, function(el) {
            if (findName(name, el) && findName(sourceClass, el.sourceClass) && findName(targetClass, el.targetClass)) {
                return el;
            };
        })
        return element;
...
VQ_Elem.prototype = {
    ...
    getElemInfo: function() {
        ...
        return {
            ...
            URI: this.fullName,
            Namespace: this.ontology.namespace,
            Prefix: this.ontology.prefix,
            DefaultNamespace: this.schema.namespace,
            triplesMaps: this.triplesMaps
        };
    }
}
...
VQ_Attribute.prototype.getAttrInfo = function() {
    ...
    if (this.type) return _.extend(rez, {
        type: this.type
    });
    else return rez;
};
...
Create_VQ_Element = function(func, location) {
    var active_diagram_type_id = Diagrams.findOne({
        _id: Session.get("activeDiagram")
    })["diagramTypeId"];
    var elem_type = ElementTypes.findOne({
        name: "Class",
        diagramTypeId: active_diagram_type_id
    });
...
VQ_Element.prototype = {
    ...
    isEqualTo: function(e) {
        if (e) {
            return this.obj["_id"] == e.obj["_id"]
        } else {
            return false
        }
    },
    // --> string (ajoo diagram id)
    getDiagram_id: function() {
        return this.obj["diagramId"]
    },
    // string --> string
    ...
    getCompartmentValue: function(compartment_name) {
        ...
        var comp_type = CompartmentTypes.findOne({
            name: compartment_name,
            elementTypeId: elem_type_id
        });
        if (comp_type) {
            var comp_type_id = comp_type["_id"];
            var comp = Compartments.findOne({
                elementId: this._id(),
    ...
    getElementTypeName: function() {
        var et = ElementTypes.findOne({
            _id: this.obj["elementTypeId"]
        });
        if (et) {
            return et["name"];
        } else {
    ...
    getConditions: function() {
        return this.getMultiCompartmentValues("Conditions").map(function(c) {
            return {
                exp: c
            }
        });
    ...
    getLinks: function() {
        return _.filter(_.union(
            Elements.find({
                startElement: this.obj["_id"]
            }).map(function(link) {
                return {
                    link: new VQ_Element(link["_id"]),
                    start: false
                };
            }),
            Elements.find({
                endElement: this.obj["_id"]
            }).map(function(link) {
                return {
                    link: new VQ_Element(link["_id"]),
                    start: true
    ...
    getElements: function() {
        return {
            start: new VQ_Element(this.obj["startElement"]),
            end: new VQ_Element(this.obj["endElement"])
        };
    },
    // --> VQ_Element
    ...
    getRootDirection: function() {
        ...
        if (findRoot(this.getStartElement())) {
            return "start"
        };
        if (findRoot(this.getEndElement())) {
            return "end"
        };
        return "none";
    },
    ...
    setLinkType: function(value) {
        if (this.isLink()) {
            ...
            if (value == "NOT") {
                ...
                setOpt = "false";
                this.setCustomStyle([{
                        attrName: "elementStyle.stroke",
                        attrValue: "#ff0000"
                    },
                    {
                        attrName: "elementStyle.dash",
                        attrValue: [0, 0]
                    },
                    {
                        attrName: "startShapeStyle.stroke",
                        attrValue: "#ff0000"
                    },
                    {
                        attrName: "endShapeStyle.stroke",
                        attrValue: "#ff0000"
                    },
                ]);
                if (this.isSubQuery()) {
                    ...
                    if (root_dir == "start") {
                        this.setCustomStyle([{
                            attrName: "startShapeStyle.fill",
                            attrValue: "#ff0000"
                        }, ]);
                    } else if (root_dir == "end") {
                        this.setCustomStyle([{
                            attrName: "endShapeStyle.fill",
                            attrValue: "#ff0000"
                        }, ]);
                    };
    ...
    setLinkQueryType: function(value) {
        if (this.isLink()) {
            ...
            if (value == "SUBQUERY") {
                ...
                if (root_dir == "start") {
                    this.setCustomStyle([{
                            attrName: "startShapeStyle.shape",
                            attrValue: "Circle"
                        },
                        //{attrName:"startShapeStyle.fill",attrValue:"#000000"},
                        {
                            attrName: "startShapeStyle.radius",
                            attrValue: 12
                        },
                        {
                            attrName: "endShapeStyle.shape",
                            attrValue: "Arrow"
                        },
                        {
                            attrName: "endShapeStyle.fill",
                        ...
                        {
                            attrName: "endShapeStyle.radius",
                            attrValue: 8
                        },
                        {
                            attrName: "elementStyle.strokeWidth",
                            attrValue: 3
                        },
                    ]);
                    if (this.isNegation()) {
    ...
    setCompartmentValue: function(comp_name, input, value) {
        var ct = CompartmentTypes.findOne({
            name: comp_name,
            elementTypeId: this.obj["elementTypeId"]
        });
        if (ct) {
            var c = Compartments.findOne({
                elementId: this._id(),
